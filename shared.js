...
const IMAGE_DB_NAME = 'citrus-kiosk-assets-v1';
const IMAGE_STORE_NAME = 'adImages';

function adAssetKey(profileId, slot) {
...
function makeIndexedDbImageRef(profileId, slot) {
  return `idb://${profileId}/${slot}`;
}
...
async function saveAdAsset(profileId, slot, dataUrl) {
  const db = await openImageDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(IMAGE_STORE_NAME, 'readwrite');
    tx.objectStore(IMAGE_STORE_NAME).put(dataUrl, adAssetKey(profileId, slot));
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error || new Error('IndexedDB write failed'));
  });
  db.close();
  return makeIndexedDbImageRef(profileId, slot);
}

async function loadAdAsset(profileId, slot) {
  const db = await openImageDb();
  const value = await new Promise((resolve, reject) => {
    const tx = db.transaction(IMAGE_STORE_NAME, 'readonly');
    const req = tx.objectStore(IMAGE_STORE_NAME).get(adAssetKey(profileId, slot));
...
async function deleteAdAsset(profileId, slot) {
  const db = await openImageDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(IMAGE_STORE_NAME, 'readwrite');
    tx.objectStore(IMAGE_STORE_NAME).delete(adAssetKey(profileId, slot));
    tx.oncomplete = () => resolve();
...
async function resolveConfigImage(config, slot) {
  const key = slot === 'top' ? 'topAdImage' : 'bottomAdImage';
  const value = config?.[key] || '';
  if (!value) return '';
  if (isIndexedDbImageRef(value)) {
    return await loadAdAsset(config.zipCode || getProfileIdFromUrl(), slot);
  }
  return value;
}
